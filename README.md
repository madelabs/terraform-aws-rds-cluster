...
| <a name="input_instance_specific_tags"></a> [instance\_specific\_tags](#input\_instance\_specific\_tags) | An object specifying tags to be added to individual clusters. This works in conjunction with database\_instance\_count variable. Provided instance number cannot exceed instance count. | <pre>list(object({<br>    instance_number = number<br>    tag_key         = string<br>    tag_value       = string<br>  }))</pre> | `[]` | no |
